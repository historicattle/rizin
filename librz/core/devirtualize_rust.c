...
typedef struct rust_known_vtable_t {
	...
	char *concrete_type;
	char *trait_name;
	RzVector /*<RustKnownMethod>*/ *methods;
} RustKnownVTable;
...
typedef struct rust_virtual_calls_t {
	HtUP *calls;
} RustVirtualCalls;
...
static void rust_known_vtable_fini(void *e, void *user) {
	...
	RZ_FREE(vtable->class_name);
	RZ_FREE(vtable->concrete_type);
	RZ_FREE(vtable->trait_name);
	rz_vector_free(vtable->methods);
}

static RZ_OWN RustVTableIndex *rust_vtable_index_new(RzCore *core) {
	RzPVector *classes = rz_analysis_class_get_all(core->analysis, false);
	if (!classes) {
		return NULL;
	}

	RustVTableIndex *index = RZ_NEW0(RustVTableIndex);
	if (!index) {
	...
	rz_pvector_foreach (classes, iter) {
		...
		rz_vector_foreach (class_vtables, analysis_vtable) {
			RustKnownVTable vtable = {
				...
				.class_name = rz_str_dup(class_name),
				.concrete_type = concrete_type(class_name),
				.trait_name = rz_str_dup(trait_name),
				.methods = rz_vector_new(sizeof(RustKnownMethod), rust_known_method_fini, NULL),
			};
			if (!vtable.class_name || !vtable.trait_name || !vtable.methods) {
...
static RustKnownVTable *rust_vtable_by_addr(RustVTableIndex *index, ut64 addr) {
	bool found = false;
	return index ? ht_up_find(index->by_addr, addr, &found) : NULL;
}
...
static void rust_virtual_calls_init(RustVirtualCalls *calls) {
	calls->calls = ht_up_new(NULL, NULL);
}

static void rust_virtual_calls_add(RzCore *core, RustVirtualCalls *calls, ut64 call_addr, RustKnownMethod *method) {
	if (!calls || !calls->calls || !method) {
		return;
	}
	char *fallback = NULL;
	const char *method_name = method->real_name;
	if (RZ_STR_ISEMPTY(method_name)) {
		method_name = method->name;
	}
	if (RZ_STR_ISEMPTY(method_name)) {
		fallback = rz_str_newf("rust.vtable.0x%08" PFMT64x ".slot.0x%" PFMT64x,
			method->vtable->addr, method->offset);
	...
	bool found = false;
	RzSetS *set = ht_up_find(calls->calls, call_addr, &found);
	if (!found) {
		set = rz_set_s_new(HT_STR_DUP);
		if (!set || !ht_up_insert(calls->calls, call_addr, set)) {
			rz_set_s_free(set);
			free(fallback);
			return;
		}
...
static void rust_virtual_calls_fini(RzCore *core, RustVirtualCalls *calls) {
	if (!calls || !calls->calls) {
		return;
	}
	ht_up_foreach(calls->calls, rust_virtual_comment_emit, core);
	ht_up_foreach(calls->calls, rust_virtual_call_set_free, NULL);
	ht_up_free(calls->calls);
	calls->calls = NULL;
}
...
static RZ_NULLABLE RustAnyVTable *devirtualize_step(RzCore *core, RzAnalysisOp *op, RustVTableIndex *vtable_index, RustVirtualCalls *virtual_calls, RzVector /*<RustAnyVTable>*/ *any_vtables, RZ_NULLABLE const RustCallSeed *seed) {
	...
	if (op_is_vtable_slot_dispatch(core, op)) {
		...
		if (is_real_reg_name(core, op->ireg)) {
			...
			ut64 scale = op->scale ? op->scale : 1;
...
static void devirtualize_rust_function(RzCore *core, RzAnalysisFunction *function, RustVTableIndex *vtable_index, RzVector /*<RustAnyVTable>*/ *any_vtables, RZ_NULLABLE const RustCallSeed *seed, RustVirtualCalls *virtual_calls) {
	ut64 start = function->addr;
	ut64 end = rz_analysis_function_max_addr(function);
...
static void collect_caller_args_from_site(RzCore *core, RzAnalysisFunction *caller, ut64 call_addr, const RustCallSeed *args, RzVector /*<RustCallSeed>*/ *seeds) {
...
static RZ_OWN RzVector /*<RustCallSeed>*/ *collect_caller_args(RzCore *core, RzAnalysisFunction *function, const RustCallSeed *args) {
...
static void devirtualize_rust_trait_object(RzCore *core) {
	...
	RustVTableIndex *vtable_index = rust_vtable_index_new(core);
	if (!vtable_index) {
		return;
	}
	RustCallSeed args = { 0 };
	...
	RustVirtualCalls virtual_calls = { 0 };
	rust_virtual_calls_init(&virtual_calls);
