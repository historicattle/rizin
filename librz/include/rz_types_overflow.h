...
#endif
// There is no builtin for "short" and "unsigned short" types
#define UT16_ADD_OVFCHK(x, y) ((UT16_MAX - (x)) < (y))
#define ST16_ADD_OVFCHK(a, b) ((((b) > 0) && ((a) > ST16_MAX - (b))) || (((b) < 0) && ((a) < ST16_MIN - (b))))
// There is no builtin for "char" and "unsigned char" types
...
#define SIGNED_MUL_OVERFLOW_CHECK_BUILTIN(overflow_name, type_base, type_min, type_max) \
	static inline bool overflow_name(type_base a, type_base b) { \
...
#if HAVE___BUILTIN_MUL_OVERFLOW
#define UNSIGNED_MUL_OVERFLOW_CHECK_BUILTIN(overflow_name, type_base, type_min, type_max) \
	static inline bool overflow_name(type_base a, type_base b) { \
		return __builtin_mul_overflow_p(a, b, (__typeof__((a) * (b)))0); \
	}
#endif
...
#if HAVE___BUILTIN_MUL_OVERFLOW
SIGNED_MUL_OVERFLOW_CHECK_BUILTIN(ST32_MUL_OVFCHK, st32, ST32_MIN, ST32_MAX)
SIGNED_MUL_OVERFLOW_CHECK_BUILTIN(ST64_MUL_OVFCHK, st64, ST64_MIN, ST64_MAX)
UNSIGNED_MUL_OVERFLOW_CHECK_BUILTIN(SZT_MUL_OVFCHK, size_t, SZT_MIN, SZT_MAX)
#else
...
#if HAVE___BUILTIN_MUL_OVERFLOW
UNSIGNED_MUL_OVERFLOW_CHECK_BUILTIN(UT32_MUL_OVFCHK, ut32, UT32_MIN, UT32_MAX)
UNSIGNED_MUL_OVERFLOW_CHECK_BUILTIN(UT64_MUL_OVFCHK, ut64, UT64_MIN, UT64_MAX)
#else
