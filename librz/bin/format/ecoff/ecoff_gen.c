...
#define ECOFF_GEN_FINI_SECTION(bits) \
	static void ecoff_section_fini_##bits(void *element, void *) { \
...
#define ECOFF_GEN_FINI_LOCAL_SYMBOL(bits) \
	static void ecoff_fini_local_symbol_##bits(void *element, void *) { \
		ECoff_LocalSymbol_##bits *lsym = element; \
		free(lsym->resolved_name); \
	}

#define ECOFF_GEN_RESOLVE_SYMBOL_NAME(bits) \
	static char *ecoff_resolve_symbol_name_##bits(RzBuffer *b, const ECoff_##bits *ecoff, ECoff_LocalSymbol_##bits *lsym, bool is_external) { \
		...
		ut64 offset_end = 0; \
		if (is_external) { \
			offset_beg = ecoff->symhdr.cb_ss_ext_offset; \
			offset_end = offset_beg + ecoff->symhdr.iss_ext_max; \
		} else { \
			offset_beg = ecoff->symhdr.cb_ss_offset; \
			offset_end = offset_beg + ecoff->symhdr.iss_max; \
		} \
		/* offset from the issBase field of a file descriptor table entry */ \
		offset_beg += lsym->iss; \
		ut8 resolved[256] = { 0 }; \
		size_t leftovers = offset_end - offset_beg; \
		size_t max_size = RZ_MIN(leftovers, (sizeof(resolved) - 1)); \
...
#define ECOFF_GEN_PARSE_LOCAL_SYMBOLS(bits) \
	static bool ecoff_parse_local_symbols_##bits(RzBuffer *b, ECoff_##bits *ecoff) { \
		if (ecoff->symhdr.isym_max < 1) { \
			return true; \
		} \
		ut64 offset = ecoff->symhdr.cb_sym_offset; \
		size_t count = ecoff->symhdr.isym_max; \
		for (size_t i = 0; i < count; ++i) { \
			ut64 location = offset; \
			...
			lsym.resolved_name = ecoff_resolve_symbol_name_##bits(b, ecoff, &lsym, false); \
			if (!lsym.resolved_name) { \
				lsym.resolved_name = rz_str_newf("unknown_%" PFMT64x, location); \
			} \
...
#define ECOFF_GEN_FINI_EXTERNAL_SYMBOL(bits) \
	static void ecoff_fini_external_symbol_##bits(void *element, void *) { \
...
#define ECOFF_GEN_PARSE_EXTERNAL_SYMBOLS(bits) \
	static bool ecoff_parse_external_symbols_##bits(RzBuffer *b, ECoff_##bits *ecoff) { \
		if (ecoff->symhdr.iext_max < 1) { \
			return true; \
		} \
		ut64 offset = ecoff->symhdr.cb_ext_offset; \
		size_t count = ecoff->symhdr.iext_max; \
		for (size_t i = 0; i < count; ++i) { \
			ut64 location = offset; \
			...
			esym.asym.resolved_name = ecoff_resolve_symbol_name_##bits(b, ecoff, &esym.asym, true); \
			if (!esym.asym.resolved_name) { \
				esym.asym.resolved_name = rz_str_newf("unknown_%" PFMT64x, location); \
			} \
...
#define ECOFF_GEN_INIT_FILE_DESCRIPTOR_ENTRY(bits) \
	static bool ecoff_init_file_descriptor_entry_##bits(RzBuffer *b, ut64 *offset, ECoff_FileDescEntry_##bits *fde, const bool big_endian) { \
		ut16 bit_fields = 0; \
...
#define ECOFF_GEN_PARSE_FILE_DESCRIPTOR_ENTRIES(bits) \
	static bool ecoff_parse_file_descriptor_entries_##bits(RzBuffer *b, ECoff_##bits *ecoff) { \
		if (ecoff->symhdr.ifd_max < 1) { \
			return true; \
		} \
		ut64 offset = ecoff->symhdr.cb_fd_offset; \
		size_t count = ecoff->symhdr.ifd_max; \
		for (size_t i = 0; i < count; ++i) { \
			ECoff_FileDescEntry_##bits fde = { 0 }; \
			if (!ecoff_init_file_descriptor_entry_##bits(b, &offset, &fde, ecoff->big_endian)) { \
				return false; \
			} \
...
#define ECOFF_GEN_INIT_PROC_DESCRIPTOR_ENTRY(bits) \
	static bool ecoff_init_proc_descriptor_entry_##bits(RzBuffer *b, ut64 *offset, ECoff_ProcDescEntry_##bits *pde, const bool big_endian) { \
		ut32 bit_fields = 0; \
...
#define ECOFF_GEN_PARSE_PROC_DESCRIPTOR_ENTRIES(bits) \
	static bool ecoff_parse_proc_descriptor_entries_##bits(RzBuffer *b, ECoff_##bits *ecoff) { \
		if (ecoff->symhdr.ipd_max < 1) { \
			return true; \
		} \
		ut64 offset = ecoff->symhdr.cb_pd_offset; \
		size_t count = ecoff->symhdr.ipd_max; \
		for (size_t i = 0; i < count; ++i) { \
			ECoff_ProcDescEntry_##bits pde = { 0 }; \
			if (!ecoff_init_proc_descriptor_entry_##bits(b, &offset, &pde, ecoff->big_endian)) { \
				return false; \
			} \
...
#define ECOFF_GEN_SECTION_TO_STRUCTURE(bits) \
	static bool ecoff_section_to_structure_##bits(const ECoff_Section_##bits *section, RzStructuredData *parent) { \
		return rz_structured_data_map_add_string(parent, "s_name", section->resolved_name) && \
...
#define ECOFF_GEN_FILE_DESCR_ENTRY_TO_STRUCTURE(bits) \
	static bool ecoff_file_descr_entry_to_structure_##bits(const ECoff_FileDescEntry_##bits *fde, RzStructuredData *parent) { \
...
#define ECOFF_GEN_FILE_DESCR_ENTRIES_TO_STRUCTURE(bits) \
	static bool ecoff_file_descr_entries_to_structure_##bits(const ECoff_##bits *ecoff, RzStructuredData *parent) { \
		...
		rz_vector_foreach (ecoff->file_descs, fde) { \
			if (!ecoff_file_descr_entry_to_structure_##bits(fde, fdes)) { \
				return false; \
			} \
		} \
		return true; \
...
#define ECOFF_GEN_PROC_DESCR_ENTRY_TO_STRUCTURE(bits) \
	static bool ecoff_proc_descr_entry_to_structure_##bits(const ECoff_ProcDescEntry_##bits *pde, RzStructuredData *parent) { \
		...
		return rz_structured_data_map_add_unsigned(pde_info, "adr", pde->adr, true) && \
			...
			rz_structured_data_map_add_signed(pde_info, "iline", pde->iline) && \
			...
			rz_structured_data_map_add_signed(pde_info, "ln_low", pde->ln_low) && \
			rz_structured_data_map_add_signed(pde_info, "ln_high", pde->ln_high) && \
...
#define ECOFF_GEN_PROC_DESCR_ENTRIES_TO_STRUCTURE(bits) \
	static bool ecoff_proc_descr_entries_to_structure_##bits(const ECoff_##bits *ecoff, RzStructuredData *parent) { \
		RzStructuredData *fdes = rz_structured_data_map_add_array(parent, "pdes"); \
		if (!fdes) { \
			return false; \
		} \
		const ECoff_ProcDescEntry_##bits *pde; \
		rz_vector_foreach (ecoff->proc_descs, pde) { \
			if (!ecoff_proc_descr_entry_to_structure_##bits(pde, fdes)) { \
				return false; \
			} \
		} \
		return true; \
...
#define ECOFF_GEN_LOCAL_SYMBOL_TO_STRUCTURE(bits) \
	static bool ecoff_local_symbol_to_structure_##bits(const ECoff_LocalSymbol_##bits *lsym, RzStructuredData *sym) { \
		...
		return rz_structured_data_map_add_string(sym, "name", lsym->resolved_name) && \
			rz_structured_data_map_add_unsigned(sym, "value", lsym->value, true) && \
			rz_structured_data_map_add_signed(sym, "iss", lsym->iss) && \
			...
			rz_structured_data_map_add_unsigned(sym, "index", lsym->index, true); \
	}
...
#define ECOFF_GEN_EXTERNAL_SYMBOL_TO_STRUCTURE(bits) \
	static bool ecoff_external_symbol_to_structure_##bits(const ECoff_ExternSymbol_##bits *esym, RzStructuredData *parent) { \
		...
		return ecoff_local_symbol_to_structure_##bits(&esym->asym, asym) && \
			...
			rz_structured_data_map_add_signed(sym, "ifd", esym->ifd); \
	}
...
#define ECOFF_GEN_FUNCTIONS(bits) \
	...
	ECOFF_GEN_INIT_FILE_DESCRIPTOR_ENTRY(bits) \
	ECOFF_GEN_PARSE_FILE_DESCRIPTOR_ENTRIES(bits) \
	ECOFF_GEN_INIT_PROC_DESCRIPTOR_ENTRY(bits) \
	...
	ECOFF_GEN_FILE_DESCR_ENTRY_TO_STRUCTURE(bits) \
	ECOFF_GEN_FILE_DESCR_ENTRIES_TO_STRUCTURE(bits) \
	ECOFF_GEN_PROC_DESCR_ENTRY_TO_STRUCTURE(bits) \
